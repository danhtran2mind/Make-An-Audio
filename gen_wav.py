...
def gen_wav(sampler, vocoder, prompt, ddim_steps, scale, duration, n_samples, seed=None):
    ...
    shape = [sampler.model.first_stage_model.embed_dim, 10, latent_width]
...
if __name__ == '__main__':
    ...
    vocoder = VocoderBigVGAN('useful_ckpts/bigvgan',
                            device=device)
    print("Generating audios, it may takes a long time depending on your gpu performance")
    wav_list = gen_wav(sampler, vocoder, prompt=args.prompt, ddim_steps=args.ddim_steps,
                      scale=args.scale, duration=args.duration, n_samples=args.n_samples, seed=args.seed)
