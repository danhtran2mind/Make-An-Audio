...
def process_audio_by_tsv(rank, args):
    ...
    # Use the device specified in args
    device = torch.device(args.device if args.device != 'cuda' else f'cuda:{rank}')
    mel_net = MelNet(args.__dict__)
    mel_net.to(device)
    # if args.num_gpus > 1:  # DistributedDataParallel is not needed when a module doesn't have any parameter that requires a gradient.
...
def split_list(i_list, num):
    each_num = math.ceil(len(i_list) / num)  # Fixed: Use len(i_list) to get the length of the list
    result = []
    for i in range(num):
        s = each_num * i
        e = each_num * (i + 1)
...
def parse_args():
    ...
    parser.add_argument("--device", type=str, 
                        ...
                        choices=['cuda', 'cpu'], help="Device to use for processing (cuda or cpu)")
...
if __name__ == '__main__':
    ...
    args = {
        ...
        'save_mel': True,
        'save_path': save_path,
        'device': pargs.device  # Add device to args
    }
